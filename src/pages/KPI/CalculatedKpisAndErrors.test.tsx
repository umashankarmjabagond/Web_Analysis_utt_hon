...
describe("CalculatedKpisAndErrors", () => {
  it("renders section headers", () => {
    ...
    expect(
      screen.getAllByText("Calculated KPIs"),
    ).toHaveLength(2);
  ...
  it("renders default KPI data", () => {
    ...
    expect(
      screen.getByText("KPI 1"),
    ).toBeInTheDocument();

    expect(
      screen.getByText("124.5"),
    ).toBeInTheDocument();

    expect(
      screen.getByText("KPI 8"),
    ).toBeInTheDocument();

    expect(
      screen.getByText("77.7"),
    ).toBeInTheDocument();
  });
  ...
  it("renders all KPI items", () => {
    ...
    expect(
      screen.getByText("KPI A"),
    ).toBeInTheDocument();

    expect(
      screen.getByText("KPI B"),
    ).toBeInTheDocument();

    expect(
      screen.getByText("KPI C"),
    ).toBeInTheDocument();
  });
  ...
  it("shows no errors message when errors array is empty", () => {
    ...
    expect(
      screen.getByText("No errors found"),
    ).toBeInTheDocument();
  });
  ...
  it("renders multiple error messages", () => {
    ...
    expect(
      screen.getByText("First Error"),
    ).toBeInTheDocument();

    expect(
      screen.getByText("Second Error"),
    ).toBeInTheDocument();

    expect(
      screen.getByText("Third Error"),
    ).toBeInTheDocument();
  });
